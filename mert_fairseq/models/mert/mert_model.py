...
from .transformer_encoder import (
    MultiWindowMultiheadAttention,
    MultiheadAttention_extend,
    TransformerEncoder_extend,
    TransformerSentenceEncoderLayerCausal,
    TransformerSentenceEncoderLayerExtend,
)
...
class MERTConfig(FairseqDataclass):
    ...
    mask_encode: bool = field(
        default=False, metadata={"help": "whether to mask the encoded features"}
    )
    ...
    pos_type: str = field(
        default="conv",
        metadata={"help": "type of positional embeddings to use (conv, abs, or rope)"},
    )
    rope_base: float = field(
        default=10000.0,
        metadata={"help": "RoPE frequency base when pos_type=rope"},
    )
    conv_pos: int = field(
        default=128,
        metadata={"help": "number of filters for convolutional positional embeddings"},
...
class MERTModel(BaseFairseqModel):
    def __init__(
        ...
        self.cfg = cfg
        self.embed = feature_enc_layers[-1][0] if cfg.audio_extract_type == "w2v_conv" else cfg.encoder_embed_dim
        ...
        if self.cfg.feature_extractor_cqt:
            self.embed += cfg.feature_extractor_cqt_bins 
        ...
        if (
            cfg.attention_relax > 0
            or cfg.deepnorm
            or cfg.subln
            or cfg.pos_type == "rope"
        ):
            if cfg.subln:
        ...
        if self.decoder_type != "none":
            ...
            if self.decoder_type == 'mae':
                ...
                if cfg.decoder_causal:
                    ...
                    cfg.decoder_self_attn_type != "standard"
                    or cfg.pos_type == "rope"
                ):
                    self.decoder = TransformerEncoder_extend(
                        decoder_cfg,
    ...
    def forward(
        ...
        if self.cfg.pos_type == "rope":
            positions = torch.arange(features.size(1), device=features.device)
            positions = positions.unsqueeze(0).expand(features.size(0), -1)
            if self.mask_encode and mask:
                if self.mask_type == "mae":
                    # MAE keeps tokens in shuffled order, so recover ids_keep
                    # from the inverse permutation returned by mae_masking.
                    ids_shuffle = torch.argsort(ids_restore, dim=1)
                    position_ids = ids_shuffle[:, : x.size(1)]
                else:
                    position_ids = positions[~masked_indices].view(
                        x.size(0), x.size(1)
                    )
            else:
        ...
        if position_ids is None:
            x, layer_results = self.encoder(x, padding_mask=masked_padding_mask)
        else:
        ...
        if self.decoder_type != 'none':
            ...
            if decoder_position_ids is None:
                ...
                x, dec_layer_results = self.decoder(
                    x,
                    padding_mask=padding_mask,
        ...
        if not self.skip_masked:
            ...
            if self.learnable_temp:
                ...
                logit_m_list = [
                    compute_pred_fast(proj_x_m, t[masked_indices], label_embs_list[i])
        ...
        if not self.skip_nomask and self.mask_type == "hubert":
            ...
            logit_u_list = [
                compute_pred_fast(proj_x_u, t[ids_restore], label_embs_list[i])
